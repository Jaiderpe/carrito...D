    ...
    servicesGrid.innerHTML = services.map(service => `
        <div class="card">
            ...
            <div class="card-footer">
                <button class="add-btn" onclick="addToCart(${service.id}, 'service')">+ Agregar</button>
            </div>
        </div>
    `).join('');
}
    ...
    productsGrid.innerHTML = products.map(product => `
        <div class="card">
            ...
            <div class="card-content">
                ...
                ${product.presentation ? `<div class="card-presentation">${product.presentation}</div>` : ''}
                <p class="card-description">${product.description}</p>
            </div>
            <div class="card-footer">
                <div class="price-section">
                    <div class="price-label">Precio</div>
                    ${product.price ? `<div class="price-value">$${product.price.toLocaleString('es-CO')}</div>` : '<div class="price-consult">Consultar</div>'}
                </div>
                <button class="add-btn" onclick="addToCart(${product.id}, 'product')">+ Agregar</button>
            </div>
        </div>
    `).join('');
}
...
function toggleConfirmationDialog() {
    const isActive = confirmationDialog.classList.contains('active');
...
function setupEventListeners() {
    ...
    backBtn.addEventListener('click', toggleConfirmationDialog);
    confirmBtn.addEventListener('click', () => {
        window.location.href = 'https://jazizbiologico.com/';
    });
    cancelBtn.addEventListener('click', toggleConfirmationDialog);
    confirmationOverlay.addEventListener('click', toggleConfirmationDialog);
    serviciosNav.addEventListener('click', () => {
        document.getElementById('servicesSection').scrollIntoView({ behavior: 'smooth' });
    });
    productosNav.addEventListener('click', () => {
